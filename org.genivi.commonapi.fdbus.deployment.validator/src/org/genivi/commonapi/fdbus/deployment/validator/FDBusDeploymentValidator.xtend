/* Copyright (C) 2014-2020 Bayerische Motoren Werke Aktiengesellschaft (BMW AG)
   ...
   package org.genivi.commonapi.fdbus.deployment.validator
...
class FDBusDeploymentValidator
{
	...
	val FDBUS_SPECIFICATION_TYPE = "fdbus.deployment"
    ...
    val DEFAULT_FDBUS_ENUM_WIDTH = 1
    ...
    var fdFDBusInterfacesNames = new ArrayList<String>
        ...
        for (fdepl : fdepls)
        {
            ...
            if (!deplFileName.endsWith(DEPLOYMENT_SPECIFICATION_FILE_SUFFIX))
            {
                ...
                for (fdInterface : fdepl.deployments.filter(typeof(FDInterface)))
                {
                    ...
                    if (fdInterface.spec.name !== null && fdInterface.spec.name.contains(FDBUS_SPECIFICATION_TYPE))
                    {
                        checkDuplicateInterface(fdFDBusInterfacesNames, fdInterface)
                        fdInterfaces.add(fdInterface)
                    }
                }
                for (fdProvider : ProviderUtils.getProviders(fdepl))
                {
                    if (fdProvider.spec.name !== null && fdProvider.spec.name.contains(FDBUS_SPECIFICATION_TYPE))
                ...
                for (fdTypes : fdepl.deployments.filter(typeof(FDTypes)))
                {
                    ...
                    if (fdTypes.spec.name !== null && fdTypes.spec.name.contains(FDBUS_SPECIFICATION_TYPE))
                    {
                        checkDuplicateType(fdFDBusInterfacesNames, fdTypes)
                        fdTypeCollections.addAll(fdTypes)
                    }
                }
            }
            fdFDBusInterfacesNames.clear
    ...
    private def validateEnumSize(FDElement fdElem, FEnumerationType enumType)
    {
        ...
        var FDProperty propFDBusEnumWidth
        if (properties !== null)
            propFDBusEnumWidth = properties.items.findFirst[decl.name == "FDBusEnumWidth"]
        if (propFDBusEnumWidth === null)
        {
            val fdEnum = getEnumTypeDeployments(enumType)
            if (fdEnum?.properties !== null)
                propFDBusEnumWidth = fdEnum.properties.items.findFirst[decl.name == "FDBusEnumWidth"]
        }

        var enumWidth =
            if (propFDBusEnumWidth !== null)
                (propFDBusEnumWidth.value.single as FDInteger).value
            else
                DEFAULT_FDBUS_ENUM_WIDTH
        ...
        if (maxUsedEnumValue > maxPossibleEnumValue)
        {
            if (propFDBusEnumWidth !== null)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    ...
                    propFDBusEnumWidth, null, -1, null, null)
                diagnostics.add(diag)
            }
            else
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "Enumeration type \"" + enumType.name + "\" needs a deployment with a size greater than the default deployment size of " + DEFAULT_FDBUS_ENUM_WIDTH + " byte.",
    ...
    private def validateEnumInvalidValue(FDPropertySet properties)
    {
        if (properties !== null)
        {
            val propFDBusEnumInvalidValue = properties.items.findFirst[decl.name == "FDBusEnumInvalidValue"]
            if (propFDBusEnumInvalidValue !== null)
            {
                val enumInvalidValue = BigInteger.valueOf((propFDBusEnumInvalidValue.value.single as FDInteger).value)

                var enumWidth = DEFAULT_FDBUS_ENUM_WIDTH
                val propFDBusEnumWidth = properties.items.findFirst[decl.name == "FDBusEnumWidth"]
                if (propFDBusEnumWidth !== null)
                    enumWidth = (propFDBusEnumWidth.value.single as FDInteger).value

                val maxEnumValue = getMaxValue(enumWidth)
                if (enumInvalidValue > maxEnumValue)
                {
                    var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                        "Value of \"FDBusEnumInvalidValue\" does not fit into an integer with a size of \"FDBusEnumWidth = " + enumWidth + "\".",
                        propFDBusEnumInvalidValue, null, -1, null, null)
                    diagnostics.add(diag)
                }
            }
        }
    ...
    {
        val String minLengthName = "FDBusArrayMinLength"
        val String maxLengthName = "FDBusArrayMaxLength"
        val String lengthWidthName = "FDBusArrayLengthWidth"
        ...
        val propFDBusArrayMinLength = properties.items.findFirst[it.decl.name == minLengthName]
        val propFDBusArrayMaxLength = properties.items.findFirst[it.decl.name == maxLengthName]
        val propFDBusArrayLengthWidth = properties.items.findFirst[it.decl.name == lengthWidthName]

        var fdbusArrayMinLength = getInteger(propFDBusArrayMinLength)
        var fdbusArrayMaxLength = getInteger(propFDBusArrayMaxLength)
        var fdbusArrayLengthWidth = getInteger(propFDBusArrayLengthWidth)

        if (fdbusArrayMinLength !== null && fdbusArrayMaxLength !== null && fdbusArrayMinLength == fdbusArrayMaxLength && fdbusArrayMinLength != 0)
        {
            // * If FDBusArrayLengthWidth == 1, 2 or 4 bytes, FDBusArrayMinLength and FDBusArrayMaxLength are ignored.
            //
            if (fdbusArrayLengthWidth === null)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "Array deployment with '" + minLengthName + " = " + fdbusArrayMinLength + "'; '"+ maxLengthName + " = " + fdbusArrayMaxLength + "' is missing a '" + lengthWidthName + " = 0' property.",
                    propFDBusArrayMinLength, null, -1, null, null)
                diagnostics.add(diag)
            }
            else if (fdbusArrayLengthWidth != 0)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "Array deployment with '" + minLengthName + " = " + fdbusArrayMinLength + "'; '"+ maxLengthName + " = " + fdbusArrayMaxLength + "' does not specify '" + lengthWidthName + " = 0'.",
                    propFDBusArrayLengthWidth, null, -1, null, null)
                diagnostics.add(diag)
            }
        }
        else if (fdbusArrayLengthWidth !== null && fdbusArrayLengthWidth == 0)
        {
            // * If FDBusArrayLengthWidth == 0, the array has FDBusArrayMaxLength elements.
            //
            if (fdbusArrayMaxLength === null || fdbusArrayMaxLength == 0)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
        ...
        if ((fdbusArrayLengthWidth === null || fdbusArrayLengthWidth == 0) && fdbusArrayMinLength !== null && fdbusArrayMaxLength !== null && fdbusArrayMinLength > fdbusArrayMaxLength)
        {
            var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
    ...
    private def validateMapDeployments()
    {
        fdInterfaces.forEach[
            attributes.forEach[
                if (isMap(target))
                    validateMapDeployment(it, "FDBusAttrMapMinLength", "FDBusAttrMapMaxLength", "FDBusAttrMapLengthWidth")
                else
                    warnAboutMapDeployment(it, "FDBusAttrMapMinLength", "FDBusAttrMapMaxLength", "FDBusAttrMapLengthWidth")
            ]
            ...
            fdArguments.forEach[
                if (isMap(target))
                    validateMapDeployment(it, "FDBusArgMapMinLength", "FDBusArgMapMaxLength", "FDBusArgMapLengthWidth")
                else
                    warnAboutMapDeployment(it, "FDBusArgMapMinLength", "FDBusArgMapMaxLength", "FDBusArgMapLengthWidth")
            ]
        ]
    }
    ...
    private def validateMapDeployment(FDElement fd, String minLengthName, String maxLengthName, String lengthWidthName)
    {
        ...
        val propFDBusMapMinLength = properties.items.findFirst[it.decl.name == minLengthName]
        val propFDBusMapMaxLength = properties.items.findFirst[it.decl.name == maxLengthName]
        val propFDBusMapLengthWidth = properties.items.findFirst[it.decl.name == lengthWidthName]

        var fdbusMapMinLength = getInteger(propFDBusMapMinLength)
        var fdbusMapMaxLength = getInteger(propFDBusMapMaxLength)
        var fdbusMapLengthWidth = getInteger(propFDBusMapLengthWidth)

        if (fdbusMapMinLength !== null && fdbusMapMaxLength !== null && fdbusMapMinLength == fdbusMapMaxLength && fdbusMapMinLength != 0)
        {
            // * If FDBusMapLengthWidth == 1, 2 or 4 bytes, FDBusMapMinLength and FDBusMapMaxLength are ignored.
            //
            if (fdbusMapLengthWidth === null)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "Map deployment with '" + minLengthName + " = " + fdbusMapMinLength + "'; '"+ maxLengthName + " = " + fdbusMapMaxLength + "' is missing a '" + lengthWidthName + " = 0' property.",
                    propFDBusMapMinLength, null, -1, null, null)
                diagnostics.add(diag)
            }
            else if (fdbusMapLengthWidth != 0)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "Map deployment with '" + minLengthName + " = " + fdbusMapMinLength + "'; '"+ maxLengthName + " = " + fdbusMapMaxLength + "' does not specify '" + lengthWidthName + " = 0'.",
                    propFDBusMapLengthWidth, null, -1, null, null)
                diagnostics.add(diag)
            }
        }
        else if (fdbusMapLengthWidth !== null && fdbusMapLengthWidth == 0)
        {
            // * If FDBusMapLengthWidth == 0, the map has FDBusMapMaxLength elements.
            //
            if (fdbusMapMaxLength === null || fdbusMapMaxLength == 0)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
        ...
        if ((fdbusMapLengthWidth === null || fdbusMapLengthWidth == 0) && fdbusMapMinLength !== null && fdbusMapMaxLength !== null && fdbusMapMinLength > fdbusMapMaxLength)
        {
            var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
    ...
    private def warnAboutMapDeployment(FDElement fd, String minLengthName, String maxLengthName, String lengthWidthName)
    {
        ...
        if (propFDBusMapMinLength !== null) {
            var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                "Map deployment used for none map type.",
                propFDBusMapMinLength, null, -1, null, null)
            diagnostics.add(diag)
        }
        if (propFDBusMapMaxLength !== null) {
            var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                "Map deployment used for none map type.",
                    propFDBusMapMaxLength, null, -1, null, null)
            diagnostics.add(diag)
        }
        if (propFDBusMapLengthWidth !== null) {
            var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                "Map deployment used for none map type.",
                    propFDBusMapLengthWidth, null, -1, null, null)
            diagnostics.add(diag)
    ...
    {
        val String minLengthName = "FDBusByteBufferMinLength"
        val String maxLengthName = "FDBusByteBufferMaxLength"
        val String lengthWidthName = "FDBusByteBufferLengthWidth"
        ...
        val propFDBusMinLength = properties.items.findFirst[it.decl.name == minLengthName]
        val propFDBusMaxLength = properties.items.findFirst[it.decl.name == maxLengthName]
        val propFDBusLengthWidth = properties.items.findFirst[it.decl.name == lengthWidthName]

        var fdbusMinLength = getInteger(propFDBusMinLength)
        var fdbusMaxLength = getInteger(propFDBusMaxLength)
        var fdbusLengthWidth = getInteger(propFDBusLengthWidth)

        if (fdbusMinLength !== null && fdbusMaxLength !== null && fdbusMinLength == fdbusMaxLength && fdbusMinLength != 0)
        {
            // * If FDBusByteBufferLengthWidth == 1, 2 or 4 bytes, FDBusByteBufferMinLength and FDBusByteBufferMaxLength are ignored.
            //
            if (fdbusLengthWidth === null)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "ByteBuffer deployment with '" + minLengthName + " = " + fdbusMinLength + "'; '"+ maxLengthName + " = " + fdbusMaxLength + "' is missing a '" + lengthWidthName + " = 0' property.",
                    propFDBusMinLength, null, -1, null, null)
                diagnostics.add(diag)
            }
            else if (fdbusLengthWidth != 0)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "ByteBuffer deployment with '" + minLengthName + " = " + fdbusMinLength + "'; '"+ maxLengthName + " = " + fdbusMaxLength + "' does not specify '" + lengthWidthName + " = 0'.",
                    propFDBusLengthWidth, null, -1, null, null)
                diagnostics.add(diag)
            }
        }
        else if (fdbusLengthWidth !== null && fdbusLengthWidth == 0)
        {
            // * If FDBusByteBufferLengthWidth == 0, the array has FDBusByteBufferMaxLength elements.
            //
            if (fdbusMaxLength === null || fdbusMaxLength == 0)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
        ...
        if ((fdbusLengthWidth === null || fdbusLengthWidth == 0) && fdbusMinLength !== null && fdbusMaxLength !== null && fdbusMinLength > fdbusMaxLength)
        {
            var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
    ...
    {
        val String lengthName = "FDBusStringLength"
        val String lengthWidthName = "FDBusStringLengthWidth"
        ...
        val propFDBusLength = properties.items.findFirst[it.decl.name == lengthName]
        val propFDBusLengthWidth = properties.items.findFirst[it.decl.name == lengthWidthName]

        var fdbusLength = getInteger(propFDBusLength)
        var fdbusLengthWidth = getInteger(propFDBusLengthWidth)

        if (fdbusLengthWidth !== null && fdbusLengthWidth == 0)
        {
            // if lengthwidth = 0 the stringlength must be set >= 0
            //
            if (fdbusLength === null || fdbusLength < 0)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                    "String deployment specifies 'FDBusStringLengthWidth = 0', but has no 'FDBusStringLength >= 0' specified.",
                    propFDBusLengthWidth, null, -1, null, null)
                diagnostics.add(diag)
            }
        }
    ...
    {
        val String maxLengthName = "FDBusUnionMaxLength"
        val String lengthWidthName = "FDBusUnionLengthWidth"
        ...
        var fdbusLength = getInteger(propFDBusMaxLength)
        var fdbusLengthWidth = getInteger(propFDBusLengthWidth)
        ...
        if (fdbusLengthWidth !== null && fdbusLengthWidth == 0)
        {
            ...
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                    "Union deployment specifies 'FDBusUnionLengthWidth = 0', but has no 'FDBusUnionMaxLength >= 0' specified.",
                    propFDBusLengthWidth, null, -1, null, null)
                diagnostics.add(diag)
            }
        }
        ...
        for (fdType : fdTypeDeployments)
        {
            if (!(fdType instanceof FDArray))
            {
                fdType.properties.items.forEach[prop|
                    ...
                    // already disallows to specify 'FDBusArray' deployment properties for none array types.
                    ...
                    if (propName !== null && propName.startsWith("FDBusArray"))
                    {
                        var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
    ...
    /**
     * - Every method, getter and setter must have a unique 'FDBusMethodID' / 'FDBusGetterID' / 'FDBusSetterID'
     *   within the interface and its base interfaces.
     *
     * - Every broadcast and attribute notifier must have a unique 'FDBusEventID' / 'FDBusNotifierID'
     *   within the interface and its base interfaces.
     *
    ...
    private def validateIds(FDInterface fdInterface)
    {
        ...
        // Method ID values for "FDBusGetterID", "FDBusSetterID" and "FDBusMethodID" must be unique for an interface and all base interfaces
        //
        ...
        // Event ID values for "FDBusNotifierID" and "FDBusEventID" must be unique for an interface and all base interfaces.
        //
    ...
    /**
     * - Every attribute must have at least one of [FDBusGetterID, FDBusSetterID, FDBusNotifierID] specified.
     *
     * - If an attribute is declared as 'readonly', it must *not* have a 'FDBusSetterID' specification.
     *
     * - If an attribute is declared as 'noSubscriptions', it must *not* have a 'FDBusNotifierID' specification.
     *
     * - If an attribute has a 'FDBusNotifierID' specification, the attribute must also have a valid event group
     *   specified within the 'FDBusNotifierEventGroups' setting.
     */
        ...
        HashMap<Integer, ArrayList<FDValue>> eventGroupIds)
    {
        ...
        {
            val propFDBusGetterID = attribute.properties.items.findFirst[it.decl.name == "FDBusGetterID"]
            val propFDBusSetterID = attribute.properties.items.findFirst[it.decl.name == "FDBusSetterID"]
            val propFDBusNotifierID = attribute.properties.items.findFirst[it.decl.name == "FDBusNotifierID"]

            val getterId = getId(propFDBusGetterID)
            val setterId = getId(propFDBusSetterID)
            val notifierId = getId(propFDBusNotifierID)
            ...
            if (getterId !== null)
            {
                // 'FDBusGetterID = 0' is allowed, if the attribute is not marked as 'noread'
            ...
            if (setterId !== null)
            {
                // 'FDBusSetterID = 0' is allowed, if the attribute is marked as 'readonly'
            ...
            if (notifierId !== null)
            {
                ...
                {
                    var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                       "Attribute \"" + getAttributeName(attribute) + "\" has a 'FDBusNotifierID' specified and the 'NoSubscriptions' keyword.",
                        attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                    diagnostics.add(diag)
                }

                // 'FDBusNotifierID = 0' is allowed, if the attribute is marked as 'noSubscriptions'
                if (notifierId > 0 || (notifierId == 0 && attribute.target.isNoSubscriptions))
                    validNotifierId = true
            }

            // It isn't valid to specify 'FDBusGetterID = 0' + 'FDBusSetterID = 0' + 'FDBusNotifierID = 0'
            if (((setterId !== null && setterId == 0) &&
                (getterId !== null && getterId == 0) &&
                (notifierId !== null && notifierId == 0))) {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "Attribute \"" + getAttributeName(attribute) + "\" has " +
                    "'FDBusGetterID' and 'FDBusSetterID' and 'FDBusNotifierID' " +
                    "all set to zero.",
                    attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                diagnostics.add(diag)
            }
            // The FDBus deployment for attributes needs to have at least one of the following properties specified
            //
            else if (!validGetterId && !validSetterId && !validNotifierId)
            {
                var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                    "Attribute \"" + getAttributeName(attribute) + "\" has no valid 'FDBusGetterID', 'FDBusSetterID' or 'FDBusNotifierID' specified.",
                    attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                diagnostics.add(diag)
            }
            else
            {
                if (!validGetterId)
                {
                    if (getterId !== null && getterId == 0 && attribute.target.noRead)
                    {
                        ...
                        var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                            "Attribute \"" + attrName + "\" has a 'FDBusGetterID = 0' specified, but the associated attribute in the interface definition is declared as 'noRead'.",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                }
                else
                {
                    if (attribute.target.noRead && getterId > 0)
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                            "Attribute \"" + attrName + "\" has a 'FDBusGetterID' specified, but the associated attribute in the interface definition is declared as 'noRead'.",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                    else if (getterId > 0 && !(getterId >= MIN_METHOD_ID_VALUE && getterId <= MAX_METHOD_ID_VALUE))
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                            "Attribute \"" + attrName + "\" declares a 'FDBusGetterID' not within range " + MIN_METHOD_ID_VALUE + " - " + MAX_METHOD_ID_VALUE + ".",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                }

                // If there is no 'FDBusSetterID' specified, check whether the respective attribute is declared as 'readonly'
                //
                if (!validSetterId)
                {
                    if (!attribute.target.readonly)
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                            "Attribute \"" + attrName + "\" has no valid 'FDBusSetterID' specified, but the associated attribute in the interface definition is not declared as 'readonly'.",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                }
                else
                {
                    if (attribute.target.readonly && setterId > 0)
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                            "Attribute \"" + attrName + "\" has a 'FDBusSetterID' specified, but the associated attribute in the interface definition is declared as 'readonly'.",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                    else if (setterId > 0 && !(setterId >= MIN_METHOD_ID_VALUE && setterId <= MAX_METHOD_ID_VALUE))
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                            "Attribute \"" + attrName + "\" declares a 'FDBusSetterID' not within range " + MIN_METHOD_ID_VALUE + " - " + MAX_METHOD_ID_VALUE + ".",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                }

                // If there is no 'FDBusNotifierID' specified, check whether the associated attribute is declared as 'noSubscriptions'
                //
                if (!validNotifierId)
                {
                    if (!attribute.target.noSubscriptions)
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                            "Attribute \"" + attrName + "\" has no valid 'FDBusNotifierID' specified, but the associated attribute in the interface definition is not declared as 'noSubscriptions'.",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                }
                else
                {
                    if (attribute.target.noSubscriptions && notifierId > 0)
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.ERROR,
                            "Attribute \"" + attrName + "\" has a 'FDBusNotifierID' specified, but the associated attribute in the interface definition is declared as 'noSubscriptions'.",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                    else if (notifierId > 0 && !(notifierId >= MIN_EVENT_ID_VALUE && notifierId <= MAX_EVENT_ID_VALUE))
                    {
                        var attrName = getAttributeName(attribute)
                        var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                            "Attribute \"" + attrName + "\" declares a 'FDBusNotifierID' not within range " + MIN_EVENT_ID_VALUE + " - " + MAX_EVENT_ID_VALUE  + ".",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                }

                // If there is a 'FDBusNotifierID' specified, check whether there is also a valid event group specified.
                //
                if (validNotifierId)
                {
                    var createdWarning = false
                    var validEventGroup = false
                    var propEventGroups = attribute.properties.items.findFirst[it.decl.name == "FDBusNotifierEventGroups"]
                    if (propEventGroups === null)
                        propEventGroups = attribute.properties.items.findFirst[it.decl.name == "FDBusEventGroups"]
                    if (propEventGroups !== null)
                    {
                    ...
                    {
                        var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                            "Attribute \"" + getAttributeName(attribute) + "\" has no valid 'FDBusNotifierEventGroups' specified.",
                            attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                        diagnostics.add(diag)
                    }
                }

                // Duplicate 'FDBusGetterId = 0' are valid and Ok.
                if (validGetterId && getterId > 0)
                {
                    var id = (propFDBusGetterID.value.single as FDInteger).value
                    var props = methodIds.get(id)
                    if (props === null) {
                        props = new ArrayList<FDProperty>
                        methodIds.put(id, props)
                    }
                    props.add(propFDBusGetterID)
                }

                // Duplicate 'FDBusSetterId = 0' are valid and Ok because a interface could contain
                // multiple readonly attributes with FDBusSetterId set to 0
                if (validSetterId && setterId > 0)
                {
                    var id = (propFDBusSetterID.value.single as FDInteger).value
                    var props = methodIds.get(id)
                    if (props === null) {
                        props = new ArrayList<FDProperty>
                        methodIds.put(id, props)
                    }
                    props.add(propFDBusSetterID)
                }

                // Duplicate 'FDBusNotifierId = 0' are valid and Ok because a interface could contain
                // multiple noSubscription attributes with FDBusNotifierId set to 0
                if (validNotifierId && notifierId > 0)
                {
                    var id = (propFDBusNotifierID.value.single as FDInteger).value
                    var props = eventIds.get(id)
                    if (props === null) {
                        props = new ArrayList<FDProperty>
                        eventIds.put(id, props)
                    }
                    props.add(propFDBusNotifierID)
                }
                ...
                val propEventGroups = attribute.properties.items.findFirst[it.decl.name == "FDBusEventGroups"]
                if (propEventGroups !== null)
                {
                    var attrName = getAttributeName(attribute)
                    var diag = new FeatureBasedDiagnostic(Diagnostic.WARNING,
                        "Deprecated: Attribute \"" + attrName + "\" is using 'FDBusEventGroups', use 'FDBusNotifierEventGroups'.",
                        attribute, FD_ATTRIBUTE__TARGET, -1, null, null)
                    diagnostics.add(diag)
                }
            }
    ...
    /**
     * - Every method must have a 'FDBusMethodID [1..32767]' specified.
     */
    private def validateInterfaceMethods(FDInterface fdInterface, HashMap<Integer, ArrayList<FDProperty>> methodIds)
    {
        ...
        {
            var propMethodId = fdMethod.properties.items.findFirst[it.decl.name == "FDBusMethodID"]
    ...
    /**
     * - Every broadcast must have a 'FDBusEventID [32769..65534]' specified.
     *
     * - Every broadcast must have a valid event group specified within the 'FDBusEventGroups' setting.
     *
     */
    ...
    )
    {
        for (FDBroadcast fdBroadcast : fdInterface.broadcasts)
        {
            // Every broadcast must have a 'FDBusEventID [32769..65534]' specified.
            //
            var propEventId = fdBroadcast.properties.items.findFirst[it.decl.name == "FDBusEventID"]
            ...
            var createdWarning = false
            var validEventGroup = false
            var propEventGroups = fdBroadcast.properties.items.findFirst[it.decl.name == "FDBusEventGroups"]
            if (propEventGroups !== null)
            {
                for (fdEventGroup : propEventGroups.value.array.values)
                {
...
//    }
     ...
     * Returns false in case of two FDProperty parameters have the same value one being "FDBusGetterID" and the other "FDBusSetterID"
    ...
    private def boolean verifyIdsValueAndName(ArrayList<FDProperty> props)
    {
        ...
        if (props.size == 2)
        {
            ...
            if (!firstProperValueFromList.equals(secondProperValueFromList) && !firstProperValueFromList.equals('FDBusMethodID') && !secondProperValueFromList.equals('FDBusMethodID'))
            {
                return false
            }
        }
