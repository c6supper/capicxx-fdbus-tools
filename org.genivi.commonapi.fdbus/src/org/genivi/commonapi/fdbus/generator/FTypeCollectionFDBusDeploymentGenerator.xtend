...
class FTypeCollectionFDBusDeploymentGenerator {
    ...
    def generateTypeCollectionDeployment(FTypeCollection tc, IFileSystemAccess fileSystemAccess,
        ...
        if(FPreferencesFDBus::getInstance.getPreference(PreferenceConstantsFDBus::P_GENERATE_CODE_FDBUS, "true").equals("true")) {
            fileSystemAccess.generateFile(tc.fdbusDeploymentHeaderPath, IFileSystemAccess.DEFAULT_OUTPUT,
                tc.generateDeploymentHeader(deploymentAccessor, modelid))
            fileSystemAccess.generateFile(tc.fdbusDeploymentSourcePath, IFileSystemAccess.DEFAULT_OUTPUT,
    ...
    def private generateDeploymentHeader(FTypeCollection _tc,
        ...
        #ifndef «_tc.defineName.toUpperCase»_FDBUS_DEPLOYMENT_HPP_
        #define «_tc.defineName.toUpperCase»_FDBUS_DEPLOYMENT_HPP_

        «val DeploymentHeaders = _tc.getDeploymentInputIncludes(_accessor)»
        «FOR deploymentHeader : DeploymentHeaders.sort»
            «IF !deploymentHeader.equals(fdbusDeploymentHeaderPath(_tc))»
        ...
        #endif // «_tc.defineName.toUpperCase»_FDBUS_DEPLOYMENT_HPP_
    '''
    ...
    def protected String generateArrayDeploymentType(FTypeRef _elementType, int _indent, PropertyAccessor _accessor) {
        var String deployment = generateIndent(_indent) + "CommonAPI::FDBus::ArrayDeployment<\n"
    ...
    def protected dispatch String generateDeploymentType(FEnumerationType _enum, int _indent, PropertyAccessor _accessor) {
        if (_indent == 0)
            return generateIndent(_indent) + "CommonAPI::FDBus::EnumerationDeployment<"
    ...
    def protected dispatch String generateDeploymentType(FMapType _map, int _indent, PropertyAccessor _accessor) {
        var String deployment = generateIndent(_indent) + "CommonAPI::FDBus::MapDeployment<\n"
    ...
    def protected dispatch String generateDeploymentType(FIntegerInterval _interval, int _indent, PropertyAccessor _accessor) {
        ...
        deployment += "CommonAPI::FDBus::IntegerDeployment<int32_t>"
        return deployment;
    }
    ...
    def protected dispatch String generateDeploymentType(FStructType _struct, int _indent, PropertyAccessor _accessor) {
        ...
        if (elements.length == 0) {
            ...
            deployment += "CommonAPI::FDBus::StructDeployment<\n"
            for (e : elements) {
    ...
    def protected dispatch String generateDeploymentType(FUnionType _union, int _indent, PropertyAccessor _accessor) {
        ...
        if (elements == 0) {
            ...
            deployment += "CommonAPI::FDBus::VariantDeployment<\n"
            for (e : elements) {
                var PropertyAccessor overwriteAccessor = _accessor.getOverwriteAccessor(e)
                if (e.array) {
                    deployment = deployment + generateArrayDeploymentType(e.type, _indent + 1, overwriteAccessor)
    ...
    def protected dispatch String generateDeploymentType(FBasicTypeId _type, int _indent, PropertyAccessor _accessor) {
        ...
        if (_type == FBasicTypeId.STRING)
            deployment = deployment + "CommonAPI::FDBus::StringDeployment"
        else if (_type == FBasicTypeId.BYTE_BUFFER)
            deployment = deployment + "CommonAPI::FDBus::ByteBufferDeployment"
    ...
    def protected dispatch String getDeploymentParameter(FEnumerationType _enum, EObject _source, FTypeCollection _tc, PropertyAccessor _accessor) {
        ...
        var Integer baseType = _accessor.getFDBusEnumWidthHelper(_source)
        if (baseType === null)
            baseType = _accessor.getFDBusEnumWidth(_enum)

        var Integer bitWidth = _accessor.getFDBusEnumBitWidthHelper(_source)
        if (bitWidth === null)
            bitWidth = _accessor.getFDBusEnumBitWidthHelper(_enum)
        ...
        var Integer invalidValue = _accessor.getFDBusEnumInvalidValueHelper(_source)
        if (invalidValue === null)
            invalidValue = _accessor.getFDBusEnumInvalidValue(_enum)
        if (invalidValue !== null)
    ...
    def protected dispatch String getDeploymentParameter(FIntegerInterval _interval, EObject _source, FTypeCollection _tc, PropertyAccessor _accessor) {
        var String parameter = ""
        var Integer bitWidth = _accessor.getFDBusIntegerBitWidthHelper(_source)
        if (bitWidth !== null)
            parameter += bitWidth.toString
        else
            parameter += 32
        val Integer invalidValue = _accessor.getFDBusIntegerInvalidValueHelper(_source)
        if (invalidValue !== null) {
            parameter += ", " + invalidValue.toString
        }
        return parameter
    ...
    def protected dispatch String getDeploymentParameter(FStructType _struct, EObject _source, FTypeCollection _tc, PropertyAccessor _accessor) {
        ...
        var Integer lengthWidth = _accessor.getFDBusStructLengthWidthHelper(_struct)
        if (lengthWidth !== null)
    ...
    def protected String getDerivedDeploymentParameter(FStructType _struct, FTypeCollection _tc, PropertyAccessor _accessor) {
        ...
        if(_struct.base !== null) { // need to use the accessor for the base struct !
            var baseAccessor = getFDBusAccessor(_struct.base.eContainer as FTypeCollection)
    ...
    def protected dispatch String getDeploymentParameter(FUnionType _union, EObject _source, FTypeCollection _tc, PropertyAccessor _accessor) {
        ...
        var Integer lengthWidth = _accessor.getFDBusUnionLengthWidthHelper(_union)
        if (lengthWidth !== null)
            parameter += lengthWidth.toString + ", "
        else
            parameter += "4, "

        var Integer typeWidth = _accessor.getFDBusUnionTypeWidthHelper(_union)
        if (typeWidth !== null)
            parameter += typeWidth.toString + ", "
        else
            parameter += "4, "

        var Boolean defaultOrder = _accessor.getFDBusUnionDefaultOrderHelper(_union)
        if (defaultOrder !== null)
            parameter += defaultOrder.toString + ", "
        else
            parameter += "true, "

        var Integer maxLength = _accessor.getFDBusUnionMaxLengthHelper(_union)
        if (maxLength !== null)
    ...
    def protected dispatch String getDeploymentParameter(FBasicTypeId _typeId, EObject _source, FTypeCollection _tc, PropertyAccessor _accessor) {
        ...
        if (_typeId == FBasicTypeId.STRING) {
            val Integer length = _accessor.getFDBusStringLength(_source)
            if (length !== null)
                parameter += length.toString + ", "
            else
                parameter += "0, "

            val Integer lengthWidth = _accessor.getFDBusStringLengthWidth(_source)
            if (lengthWidth !== null)
                parameter += lengthWidth.toString + ", "
            else
                parameter += "4, "

            val PropertyAccessor.FDBusStringEncoding encoding
                = _accessor.getFDBusStringEncoding(_source)
            if (encoding !== null)
                parameter += "CommonAPI::FDBus::StringEncoding::" + encoding.toString.toUpperCase
            else
                parameter += "CommonAPI::FDBus::StringEncoding::UTF8"
    ...
    def protected String getByteBufferDeploymentParamter(EObject _buffer, EObject _source, PropertyAccessor _accessor) {
        var String parameter = ""
        var Integer minLength = _accessor.getFDBusByteBufferMinLength(_source)
        if (minLength === null && _buffer != _source)
            minLength = _accessor.getFDBusByteBufferMinLength(_buffer)
        if (minLength !== null)
            parameter += minLength.toString + ", "
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_MIN_LENGTH.toString + ", "

        var Integer maxLength = _accessor.getFDBusByteBufferMaxLength(_source)
        if (maxLength === null && _buffer != _source)
            maxLength = _accessor.getFDBusByteBufferMaxLength(_buffer)
        if (maxLength !== null)
            parameter += maxLength.toString + ", "
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_MAX_LENGTH.toString + ", "

        var Integer lengthWidth = _accessor.getFDBusByteBufferLengthWidth(_source)
        if (lengthWidth === null && _buffer != _source)
            lengthWidth = _accessor.getFDBusByteBufferLengthWidth(_buffer)
        if (lengthWidth !== null)
            parameter += lengthWidth.toString
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_LENGTH_WIDTH.toString
    ...
    def protected String getArrayDeploymentParameter(EObject _array, EObject _source, FTypeCollection _tc, PropertyAccessor _accessor) {
        var String parameter = ""
        var Integer minLength = _accessor.getFDBusArrayMinLengthHelper(_source)
        if (minLength === null && _array != _source)
            minLength = _accessor.getFDBusArrayMinLengthHelper(_array)
        if (minLength !== null)
            parameter += minLength.toString + ", "
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_MIN_LENGTH.toString + ", "          

        var Integer maxLength = _accessor.getFDBusArrayMaxLengthHelper(_source)
        if (maxLength === null && _array != _source)
            maxLength = _accessor.getFDBusArrayMaxLengthHelper(_array)
        if (maxLength !== null)
            parameter += maxLength.toString + ", "
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_MAX_LENGTH.toString + ", "           

        var Integer lengthWidth = _accessor.getFDBusArrayLengthWidthHelper(_source)
        if (lengthWidth === null && _array != _source)
            lengthWidth = _accessor.getFDBusArrayLengthWidthHelper(_array)
        if (lengthWidth !== null)
            parameter += lengthWidth.toString
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_LENGTH_WIDTH.toString
    ...
    def protected String getMapDeploymentParameter(EObject _map, EObject _source, FTypeCollection _tc, PropertyAccessor _accessor) {
        var String parameter = ""
        var Integer minLength = _accessor.getFDBusMapMinLengthHelper(_source)
        if (minLength === null && _map != _source)
            minLength = _accessor.getFDBusMapMinLengthHelper(_map)
        if (minLength !== null)
            parameter += minLength.toString + ", "
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_MIN_LENGTH.toString            

        var Integer maxLength = _accessor.getFDBusMapMaxLengthHelper(_source)
        if (maxLength === null && _map != _source)
            maxLength = _accessor.getFDBusMapMaxLengthHelper(_map)
        if (maxLength !== null)
            parameter += maxLength.toString + ", "
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_MAX_LENGTH.toString + ", "          

        var Integer lengthWidth = _accessor.getFDBusMapLengthWidthHelper(_source)
        if (lengthWidth === null && _map != _source)
            lengthWidth = _accessor.getFDBusMapLengthWidthHelper(_map)
        if (lengthWidth !== null)
            parameter += lengthWidth.toString
        else
            parameter += FrancaFDBusDeploymentAccessorHelper.FDBUS_DEFAULT_LENGTH_WIDTH.toString + ", "           
