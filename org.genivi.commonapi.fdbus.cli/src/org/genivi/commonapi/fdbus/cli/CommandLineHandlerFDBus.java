 ...
 */
public class CommandLineHandlerFDBus extends AbstractCommandLineHandler implements ICommandLineHandler
{

    public static final String      FILE_EXTENSION_FDEPL = "fdepl";
    public static final String      FILE_EXTENSION_FIDL  = "fidl";
    private FDBusCommandlineToolMain cliTool;

    public CommandLineHandlerFDBus()
    {
    	cliTool = new FDBusCommandlineToolMain();
    }
    ...
    public int excute(CommandLine parsedArguments)
    {
		...
		if(files.size() > 0 && files.get(0) != null) {
			...
			if(file.endsWith(FILE_EXTENSION_FDEPL)) {
				...
				ConsoleLogger.printLog("Executing CommonAPI FDBus Code Generation...\n");
                ...
                return cliTool.generateFDBus(files);
            }
            else
            {
